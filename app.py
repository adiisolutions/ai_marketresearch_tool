...
TTS_VOICE_OPTIONS = {
    ...
    'Female': 'com.apple.speech.synthesis.voice.samantha'
}
MODELS = {
...
def text_to_speech(text: str):
    """Convert text to speech using gTTS (works in cloud environments)"""
    try:
        ...
        audio_tag = f'<audio autoplay="true" controls src="data:audio/wav;base64,{audio_base64}">'
    ...
    with st.container():
        ...
        with col1:
            if st.button("🔊 Listen to Summary", use_container_width=True):
                text_to_speech(st.session_state.summary_result)
        with col2:
            st.download_button(
